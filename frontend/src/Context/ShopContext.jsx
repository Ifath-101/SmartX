...
const getDefaultCart = () => {
  //Initializing a empty cart assuming there
  let cart = {}; //there are 300 products in the store
  for (let index = 0; index < 300 + 1; index++) {
    //for each product id (0-300) the no of items initialized to 0
    cart[index] = 0;
  }
  return cart;
};
...
const ShopContextProvider = (props) => {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("auth-token")) {
      // checks for a token in the browser’s localStorage
      fetch("http://localhost:5000/getcart", {
        //If a user is logged in, there will be an auth-token saved
